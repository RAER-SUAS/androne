...
public class SRICPush extends Thread
{
	...
	String reconnectTimer = Backend.prefs.getString("sric_reconnectTimer", "");
	String ftpUsername = Backend.prefs.getString("sric_ftpUsername","");
	String ftpPassword = Backend.prefs.getString("sric_ftpPassword","");
	String message;
	...
	long timeoutTime = (Integer.parseInt(reconnectTimer) * 100);
	...
	public void run()
	{
		...
		{
				System.out.println("TRYING TO POST FILE");
				try
				{
					...
					String remoteFile = remoteDirectory + remoteFilename;

					File localDirectory = new File(Environment.DIRECTORY_PICTURES);
					InputStream inputStream = new FileInputStream(localDirectory + remoteFilename);
		...
		Backend.wifi.disconnect();
		Backend.wifi.setWifiEnabled(false);
		Backend.wifi.setWifiEnabled(true);
